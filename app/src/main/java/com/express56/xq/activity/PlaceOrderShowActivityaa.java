...
public class PlaceOrderShowActivityaa extends BaseActivity {
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG:
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        HttpHelper.sendRequest_payComplete(PlaceOrderShowActivityaa.this, RequestID.REQ_PAY_SUCCESS, sp.getUserInfo().token, resultInfo, dialog);
                    } else {
                        // 该笔订单真实的支付结果，需要依赖服务端的异步通知。
                        Toast.makeText(PlaceOrderShowActivityaa.this, "支付失败", Toast.LENGTH_SHORT).show();
                    }
                    break;
                default:
                    break;
                    ...
                    if (code == 9) {
                        if (object != null && object.containsKey("result")) {
                            ...
                            Runnable payRunnable = new Runnable() {
                                ...
                                @Override
                                public void run() {
                                    PayTask alipay = new PayTask(PlaceOrderShowActivityaa.this);
                    ...
                    if (code == 9) {
                        new Handler().post(new Runnable() {
                            ...
                            public void run() {
                                ...
                                Toast.makeText(PlaceOrderShowActivityaa.this, "支付成功", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else if (code == 0) {
                        showReloginDialog();
